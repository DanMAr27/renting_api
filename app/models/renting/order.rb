...
module Renting
  class Order < ApplicationRecord
    ...
    def available_action_buttons
      ...
      case status
      when "created"
        actions << { name: "submit_authorization", label: "Enviar a Autorización", endpoint: "POST /api/v1/renting/orders/#{id}/submit", primary: true }
        actions << { name: "cancel", label: "Cancelar", endpoint: "POST /api/v1/renting/orders/#{id}/cancel", destructive: true }
      when "pending_authorization"
        actions << { name: "approve", label: "Aprobar", endpoint: "POST /api/v1/renting/orders/#{id}/approve", primary: true }
        actions << { name: "reject", label: "Rechazar", endpoint: "POST /api/v1/renting/orders/#{id}/reject", destructive: true }
      when "authorized"
        actions << { name: "send_to_supplier", label: "Enviar a Proveedor", endpoint: "POST /api/v1/renting/orders/#{id}/send_to_supplier", primary: true }
      when "pending_supplier_confirmation"
        actions << { name: "confirm_by_supplier", label: "Confirmar por Proveedor", endpoint: "POST /api/v1/renting/orders/#{id}/confirm", primary: true }
      when "confirmed"
        actions << { name: "complete_delivery", label: "Completar Entrega", endpoint: "POST /api/v1/renting/orders/#{id}/complete_delivery", primary: true }
      end
    ...
    def related_entity_actions
      ...
      if vehicle.present?
        result[:vehicle] = {
          ...
          actions: vehicle.available_actions
        }
      end
      ...
      if contract.present?
        result[:contract] = {
          ...
          supplier_contract_number: contract.supplier_contract_number,
